...
public class addDoctor extends JFrame 
{
	...
	JLabel log;
	JButton b1,b2;
	JTextField t1,t2,t3,t4,t5,t6,t7,t8,uf;
	JPasswordField pf;
	JComboBox d1,d2,d3;
	Label mLabel;
	...
	add(t1);add(t2);add(t3);add(t4);add(t5);add(t6);add(t7);add(t8);add(uf);add(pf);
	add(d1);add(d2);add(d3);add(backIcon);add(log);
	add(b1);add(b2);
	p1.add(mLabel);
	add(p1);
	...
	public void b1ActionPerformed(ActionEvent evt) {
	 con = connection.ConnectDb();
         if (con != null) {
            ...
            String dept = (String) d2.getSelectedItem();
		...
		int count=0;
            ...
            String sql = "insert into doctor(count,date,id,name,age,gender,dept,phone,email,status,address,room,username,password) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            try {
